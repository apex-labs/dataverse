...
/**
 * @ClassName: DtvsAdminDemoHandler
 * @Author: wwd
 * @TODO:
...
@Component
@Slf4j
public class DtvsAdminDemoHandler {
    ...
    @XxlJob("DtvsAdminDemoHandler")
    public ReturnT<String> execute(String param) throws Exception {
        Long jobId = XxlJobHelper.getJobId();
        log.info("DtvsAdminDemoHandler is triggered ,param:{}, jobId:{}", param, jobId);
        ts = LocalDateTime.now().plusDays(-1L).format(DateTimeFormatter.ofPattern("yyyyMMdd"));
        log.info("DtvsAdminDemoHandler exe ts info:{}", ts);
...
//        }
    ...
    private void handlerJobSchedule(JobSchedule jobSchedule, List<ScheduleNode> scheduleNodeList, List<ScheduleEdge> scheduleEdgeList, List<NodeJobs> nodeJobsList, List<JobInstance> saveJobInstanceList, List<StorageBox> storageBoxList, List<ScheduleInstance> saveScheduleInstanceList) throws DtvsAdminException, InvalidCmdException, NoPortNodeException, InvalidConnException, InterruptedException, PortConnectionException, JsonProcessingException {
        ...
        if (CollectionUtil.isNotEmpty(scheduleNodeList)) {
            ...
            if (CollectionUtil.isNotEmpty(exeNodeJobsList)) {
                log.info("执行任务节点信息:{}", ObjectMapperUtil.toJson(exeNodeJobsList));
                    ...
                    if (Objects.nonNull(nodeJobs)) {
                        if (Objects.nonNull(nodeJobs.getJobType())) {
                            if (nodeJobs.getJobType().intValue() == JobTypeEnum.ETL_JOB.getValue()) {
                                exeEtlJob(nodeJobs, scheduleEdgeEnvList, saveJobInstanceList, storageBoxList);
                            } else if (nodeJobs.getJobType().intValue() == JobTypeEnum.BDM_JOB.getValue()) {
                                exeBdmJob(nodeJobs, scheduleEdgeEnvList, saveJobInstanceList, storageBoxList);
                            }
                        }
                    }
                }
        ...
        if (CollectionUtil.isNotEmpty(tableExtractList)) {
            ...
            for (TableExtract tableExtract : tableExtractList) {
                ...
                if (StringUtils.isNotBlank(sql)) {
                    ...
                    log.info("exe etj job response:{}", ObjectMapperUtil.toJson(diRspPacket));
                    // 将集成任务抽取表进入实例偏移结束节点
    ...
    private JobInstance getJobInstance(NodeJobs nodeJobs) {
    ...
    private void checkNodeJobsDependency(NodeJobs nodeJobs, List<ScheduleEdge> scheduleEdgeEnvList) throws DtvsAdminException {
        if (CollectionUtil.isNotEmpty(scheduleEdgeEnvList)) {
            ...
            if (CollectionUtil.isNotEmpty(scheduleEdgeList)) {
                for (ScheduleEdge scheduleEdge : scheduleEdgeList) {
                    if (scheduleEdge.getIsValid().intValue() == IsValidStatus.VALID_STATUS.getValue()) {
                        JobInstance jobInstance = jobInstanceService.getOne(Wrappers.<JobInstance>lambdaQuery().eq(JobInstance::getNodeCode, scheduleEdge.getFromNode()).eq(JobInstance::getEnv, nodeJobs.getEnv()));
                        if (Objects.nonNull(jobInstance) && jobInstance.getExeStatus().intValue() == ExeStatusEnum.EXE_FAIL.getValue()) {
                            throw new DtvsAdminException("节点任务前置依赖节点任务失败");
                        }
                    }
                }
            }
     ...
     */
    private void exeBdmJob(NodeJobs nodeJobs, List<ScheduleEdge> scheduleEdgeEnvList, List<JobInstance> saveJobInstanceList, List<StorageBox> storageBoxList) throws DtvsAdminException, InvalidCmdException, InvalidConnException, InterruptedException, NoPortNodeException, PortConnectionException, JsonProcessingException {
        checkNodeJobsDependency(nodeJobs, scheduleEdgeEnvList);
        // 保存抽取表统计数据
        List<TableStatistics> saveTableStatisticsList = new ArrayList<>();
        // 执行SQL集合
        ...
        BdmScript bdmScript = bdmScriptService.list(Wrappers.<BdmScript>lambdaQuery().eq(BdmScript::getBdmJobCode, bdmJobCode).eq(BdmScript::getEnv, env).orderByDesc(BdmScript::getCreateTime)).get(0);
        ...
        if (CollectionUtil.isNotEmpty(bdmOutTableList) && Objects.nonNull(bdmScript) && CollectionUtil.isNotEmpty(bdmInTableList)) {
            ...
            String exeSql = "";
            String[] scriptSqlArr = bdmScript.getBdmScript().split(SqlGrammarConst.SEMICOLON);
            ...
            for (BdmOutTable bdmOutTable : bdmOutTableList) {
                ...
                if (StringUtils.isNotBlank(exeSql)) {
                    ...
                    if (exeSql.startsWith(SqlGrammarConst.INSERT_OVERWRITE)) {
                        exeSql = exeSql.substring(exeSql.indexOf(SqlGrammarConst.SELECT_WITH_BLANK_LOWER));
                    } else if (exeSql.startsWith(SqlGrammarConst.INSERT_INTO_LOWER)) {
                        overwrite = Boolean.FALSE;
                        exeSql = exeSql.substring(exeSql.indexOf(SqlGrammarConst.SELECT_WITH_BLANK_LOWER));
                    }
                    TableStorage outTableStorage = tableStorageList.stream().filter(t -> t.getTableCode().equals(bdmOutTable.getTableCode()) && t.getEnv().intValue() == bdmOutTable.getEnv().intValue()).findFirst().orElse(null);
