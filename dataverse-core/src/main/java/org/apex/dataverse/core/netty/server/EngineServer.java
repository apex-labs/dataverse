...
/**
 * Engine Server， 此Server运行在Engine端
 * Odpc连接此Server发送执行命令
 *
 * @version : v1.0
 * @author : Danny.Huo
 ...
 */
@Slf4j
public class EngineServer implements IServer {
    ...
    public static EngineServer newEngineServer(ServerContext context) {
        return new EngineServer(context);
    }

    public EngineServer(ServerContext context) {
    ...
    private void startServer() {
        ...
        serverBootstrap.childHandler(new ChannelInitializer<Channel>() {
            ...
            protected void initChannel(Channel ch) {
                ...
                pipeline.addLast(new EngineHandler());
            }
        });
        ...
        try {
            ...
            feature.addListener(promise -> {
                if (promise.isSuccess()) {
                    ...
                    log.info("Engine server[port : {}] start successfully, channel id is : {}",
                            this.context.getEnv().getServerPort(), feature.channel().id());
                } else {
                    log.error("Engine server[port : {}] start failed",
    ...
    class EngineHandler extends SimpleChannelInboundHandler<Request<Packet>> {
        @Override
